// Hi-fi Action Queue — heterogeneous actions w/ confidence-driven row densities

// Action types & their visual treatment (small type tag, not full icon row)
const ACTION_TYPES = {
...
// Tag a few drafts as different action types so the page reads as a real Action Queue
const ACTIONS = (window.DRAFTS || []).map((d, i) => ({
  ...d,
  actionType: i === 1 ? "email-reply" : i === 3 ? "call" : i === 5 ? "loop-pm" : i === 6 ? "human" : "email-out",
  needsHuman: d.flag === "wrong-variant" || i === 6,
}));

const HFQueue = ({ onOpen, openId, hoverSignalDraft, setHoverDraft }) => {
    ...
    <div className="card" style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ padding: "16px 20px 12px", borderBottom: "1px solid var(--line)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <button className="btn accent sm">
          <svg width="11" height="11" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M1 6l4 4L11 2"/></svg>
      ...
      <div style={{ padding: "8px 20px", borderBottom: "1px solid var(--line)", display: "flex", alignItems: "center", gap: 8, background: "var(--surface-2)" }}>
        <span className={"cb " + (selCount > 0 ? "checked" : "")} onClick={() => setSelected(selCount > 0 ? {} : Object.fromEntries(DRAFTS.map(d => [d.id, true])))} />
        <span style={{ fontSize: 12, color: "var(--ink-3)" }}>
          {selCount > 0 ? <><span className="num" style={{ fontWeight: 600, color: "var(--ink)" }}>{selCount}</span> selected</> : "Select all"}
        </span>
        {selCount > 0 && (
          <>
        ...
        <button className="btn xs ghost">
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"><path d="M1 3h10M3 6h6M5 9h2"/></svg>
          Filter
        </button>
      </div>

      {/* Auto-sent digest — collapsible */}
      <div style={{ borderBottom: "1px solid var(--line)", background: "var(--surface-2)" }}>
        <div onClick={() => setDigestOpen(o => !o)} style={{ padding: "8px 20px", display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
          <svg width="11" height="11" viewBox="0 0 12 12" fill="none" stroke="var(--ok)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M1 6l4 4L11 2"/></svg>
          ...
          <svg width="10" height="10" viewBox="0 0 10 10" fill="none" stroke="var(--ink-4)" strokeWidth="1.6" strokeLinecap="round" style={{ transform: digestOpen ? "rotate(180deg)" : "none", transition: "transform .15s" }}><path d="M2 4l3 3 3-3"/></svg>
        </div>
        {digestOpen && (
        ...
        {ACTIONS.map((d) => {
          ...
          return (
            <div key={d.id}
              ...
              <span className={"cb " + (selected[d.id] ? "checked" : "")} onClick={(e) => { e.stopPropagation(); toggle(d.id); }} />
              <span className={"chc " + ch.code} title={ch.name}>{ch.letter}</span>
              {/* Recipient */}
              <div style={{ display: "flex", gap: 9, alignItems: "center", minWidth: 0 }}>
                <div className="av sm">{d.rec.initials}</div>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{d.rec.name}</div>
                  <div style={{ fontSize: 11.5, color: "var(--ink-3)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{d.rec.role} · {d.rec.company}</div>
                </div>
              </div>
              {/* Why */}
              <div style={{ fontSize: 12.5, color: "var(--ink-2)", lineHeight: 1.4, minWidth: 0 }}>
                {d.why}
                {d.flag && <span className="chip bad" style={{ marginLeft: 6, fontSize: 10.5, height: 18, padding: "1px 7px" }}>review</span>}
              </div>
              {/* Subject + action type tag · density driven by confidence */}
              <div style={{ minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 500, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", flex: 1, minWidth: 0 }}>
                    {d.actionType === "human" ? <span style={{ color: "var(--ink-3)", fontStyle: "italic" }}>Human-led — no draft. What do you want to do?</span> : d.subject}
                  </span>
                </div>
                {d.conf !== "ok" && d.actionType !== "human" && (
                  <div style={{ fontSize: 11.5, color: "var(--ink-3)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{d.preview}</div>
                )}
              </div>
              {/* Confidence */}
              <div style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12 }}>
                <span className={"dot " + d.conf} />
                <span style={{ color: "var(--ink-2)" }}>{d.conf === "ok" ? "High" : d.conf === "warn" ? "Medium" : "Low"}</span>
              </div>
              {/* Actions */}
              <div style={{ display: "flex", gap: 4, justifyContent: "flex-end" }} onClick={(e) => e.stopPropagation()}>
                <button className="btn xs primary" title="Approve & send (e)">
                  <svg width="11" height="11" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M1 6l4 4L11 2"/></svg>
                </button>
